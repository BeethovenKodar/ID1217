...
// - Tou don't want to call 'read' character-wise. Well, this forces 
...
EventType waitForEvent(EventDesc *event) {
    ...
    *copyDst = (char) 0;
    freeSpace += (ptr-buf);
    DebugCode(fprintf(stdout, "cmd=%s", inbuf););
    ...
    switch (inbuf[0]) {
        case 'b':
            matches = sscanf(inbuf, "b %d %d", &(event->fbp.floor), &(event->fbp.type));
            if (matches != 2) {
                event->e.str = inbuf;
                return (Error);
            } else {
        ...
        case 'p':
            matches = sscanf(inbuf, "p %d %d", &(event->cbp.cabin), &(event->cbp.floor));
            if (matches != 2) {
                event->e.str = inbuf;
                return (Error);
            } else {
        ...
        case 'f':
            matches = sscanf(inbuf, "f %d %lf", &(event->cp.cabin), &(event->cp.position));
            if (matches != 2) {
                event->e.str = inbuf;
                return (Error);
            } else {
